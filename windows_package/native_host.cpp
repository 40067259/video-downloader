...
// ---------------------------------------------------------
// Get user's home directory (Windows)
// ---------------------------------------------------------
...
int main() {
    ...
    if (home.empty()) {
        std::fprintf(stderr, "[Host] ERROR: Could not determine user home directory\n");
        return 1;
    }

    // Set working directory to where tools are located
    std::string toolsDir = home + "\\AppData\\Local\\video-downloader\\tools";
    if (_chdir(toolsDir.c_str()) != 0) {
        std::fprintf(stderr, "[Host] ERROR: chdir failed => %s\n", toolsDir.c_str());
        std::fprintf(stderr, "[Host] Please run install.bat first\n");
        return 1;
    }
    std::fprintf(stderr, "[Host] Working directory: %s\n", toolsDir.c_str());

    // Create downloads directory if needed
    g_downloadsDir = home + "\\Downloads\\VideoDownloader";
    std::string mkdirCmd = "if not exist \"" + g_downloadsDir + "\" mkdir \"" + g_downloadsDir + "\"";
